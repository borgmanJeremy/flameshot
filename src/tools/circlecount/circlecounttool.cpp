...
void CircleCountTool::process(QPainter &painter, const QPixmap &pixmap, bool recordUndo) {
    ...
    if(m_count < 10)
    {
        painter.drawText(QPoint(m_points.first.x()-bubble_size/2,m_points.first.y()+bubble_size/2), QString::number(m_count));
    }
    else
    {
        painter.drawText(QPoint(m_points.first.x()-bubble_size,m_points.first.y()+bubble_size/2), QString::number(m_count));
    }
}
